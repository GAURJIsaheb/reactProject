...
import {uploadImageToS3,deleteImageFromS3,resolveImageUrl,} from '../../s3/s3Service.js';
...
import {canCompleteTask,hasIncompleteSubtasks,SUBTASKS_INCOMPLETE_ERROR,} from '../../utils/taskRules.js';
import { normalizeWorkspaceType } from '../../utils/workspaceDefaults.js';
import {getCallerId,normalizeReminderAt,normalizeLabels,normalizeSubtasks,resolveWorkspace,ensureWorkspace,broadcastTaskState,
...
export const bulkCreateTasks = async (req, res) => {
  ...
  const { isDefaultWorkspaceType } = await import('../../utils/workspaceDefaults.js');
...
export const getAllTasks = async (req, res) => {
  const { workspaceType, workspaceId, page = 1, limit = 20 } = req.query;//with pagination
  const userId = getCallerId(req);
  if (!userId) return res.status(401).json({ error: 'Not authenticated' });

  const ws = await resolveWorkspace(userId, normalizeWorkspaceType(workspaceType), workspaceId);
  if (!ws) return res.json({ tasks: [], total: 0, page: 1, totalPages: 0 });

  const pageNum  = Math.max(1, parseInt(page, 10)  || 1);
  const limitNum = Math.min(100, parseInt(limit, 10) || 20); // max 100 per page
  ...
  const [tasks, total] = await Promise.all([
    ...
    Task.countDocuments({ workspaceId: ws.workspaceId, deleted: false }),
  ]);
...
export const deleteTask = async (req, res) => {
  const task = await Task.findOne({ taskId: req.params.id })
    .select('taskId workspaceId image')
    .lean();
...
export const bulkUpdateTasks = async (req, res) => {
  ...
  const toNotify = completionTargets.length
    ? currentTasks.filter(
        (task) => completionTargets.includes(task.taskId) && task.completed === false
      )
    : [];
  ...
  const bulkOps = updates.map(({ taskId, payload }) => {
    ...
    if (
      Object.prototype.hasOwnProperty.call(normalizedPayload, 'subtasks') &&
      hasIncompleteSubtasks(effectiveSubtasks)
    ) {
